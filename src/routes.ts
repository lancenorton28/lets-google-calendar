...
export function createRouter(opts: CreateRouterOptions): Router {
  ...
  router.get("/api/google/oauth/start", opts.requireAuth, (req, res) =>
    startOAuth(req, res, {
      config: opts.config,
      scopes: opts.scopes,
  ...
  router.get("/api/google/events", opts.requireAuth, async (req, res) => {
    try {
      ...
      const selectedCalendarId = (calendarId as string) || "primary";
      const calendar = await getAuthenticatedClient(
        opts.resolveUserId(req),
        opts.config,
        { tokenStore: opts.tokenStore },
      ...
      res.json(
        (response.data.items || []).map((event) =>
          mapGoogleEvent(event, selectedCalendarId),
        ),
      );
    } catch (error: any) {
